...
bool IsJunctionSection(const SectionInfo *section) {
  return (section->link_type & static_cast<uint32_t>(cem::message::env_model::LDLinkTypeMask::LT_ROUNDABOUT)) ||
         (section->link_type & static_cast<uint32_t>(cem::message::env_model::LDLinkTypeMask::LT_IC)) ||
         (section->link_type & static_cast<uint32_t>(cem::message::env_model::LDLinkTypeMask::LT_JCT));
}

const SectionInfo *GetFirstValidLinkedSection(const std::vector<uint64_t> &linked_section_ids, const FusionManager *fusion_mgr) {
...
void ForceUsePerceptionMap(std::vector<std::pair<uint64_t, double>> &route_snapshot, bool &use_perception,
                           const std::shared_ptr<EnvStatus> &env_status, const EnvStatus::MapSuppressionReason reason) {
  route_snapshot.clear();
  use_perception = true;
  env_status->set_hd_map_suppression_reason(reason);
}

void BuildSwitchFailureRouteSnapshot(const FusionManager *fusion_mgr, const SectionInfo *front_section,
  ...
  while (total_length < kSwitchFailureSnapshotLength) {
    const auto *successor_section = GetFirstValidLinkedSection(cur_section->successor_section_id_list, fusion_mgr);
...
bool ComputeSwitchFailureDistance(const std::vector<std::pair<uint64_t, double>> &route_snapshot, const uint64_t front_section_id,
                                  const double front_section_remaining_length, double *switch_failure_distance) {
...
void MapSourceSwitch::FinalizeMapSourceStatus() {
  if (use_perception_) {
    env_status_->set_map_source(EnvStatus::EnvSource::EnvStatus_EnvSource_PERCEPTION_MAP);
    env_status_->clear_perception_map_suppression_reason();
  } else {
    env_status_->set_map_source(EnvStatus::EnvSource::EnvStatus_EnvSource_HD_MAP);
    env_status_->clear_hd_map_suppression_reason();
  }
}

void MapSourceSwitch::SetRampFlag(bool flag) {
  is_in_ramp_range_ = flag;
}
...
void MapSourceSwitch::SetJunctionInfo(double distance_to_previous_junction, double distance_to_next_junction,
                                      double distance_to_section_over_first_junction, bool is_junction_from_hd_map) {
...
void MapSourceSwitch::SetEgoLaneMatchingInfo(const GeometryMatchResult &ego_lane_match_info) {
...
void MapSourceSwitch::SetMapMatchStatus() {
  ...
  auto update_map_match_status = [this, time_now](const uint64_t sequence_num, const double timestamp,
                                                  const cem::message::sensor::LocationLevel level) {
    map_match_status_.valid                 = true;
    map_match_status_.header.recv_timestamp = time_now;
    map_match_status_.header.sequence_num   = sequence_num;
  ...
  if (!map_match_result->has_header() || !map_match_result->has_location_level()) {
    return;
  }

  if (std::fabs(time_now - map_match_result->header().measurement_timestamp()) > kMapMatchTimestampToleranceSec) {
    return;
  }
  update_map_match_status(map_match_result->header().sequence_num(), map_match_result->header().measurement_timestamp(),
                          static_cast<cem::message::sensor::LocationLevel>(map_match_result->location_level()));
}
  ...
  if (function_mode_ == LdMapProcessor::FunctionMode::kHNOA) {
    ...
    if (latest_is_on_freeway != is_on_freeway_) {
      ...
      const bool should_switch_to_latest_scenario =
          use_perception_ ||
          (!use_perception_ && (!is_on_freeway_) && (distance_to_previous_junction_ > 90 && distance_to_next_junction_ <= 100)) ||
          (!use_perception_ && is_on_freeway_ && (distance_to_next_junction_ < 200));
...
void MapSourceSwitch::EvaluateEgoLaneMatchStatus(const GeometryMatchResult &ego_lane_match_info) {
  ...
  SD_COARSE_MATCH_LOG << "[EvaluateEgoLaneMatchStatus] GeometryMatchResult: "
                      ...
                      << ", right_offset_average=" << result.right_offset_average << ", right_offset_max=" << result.right_offset_max
  ...
  env_status_->mutable_left_laneline_match_info()->set_average_offset(result.left_offset_average);
  env_status_->mutable_left_laneline_match_info()->set_max_offset(result.left_offset_max);
}

void MapSourceSwitch::HNOAMapSourceSwitch() {
  if (is_in_area_should_use_hdmap_) {
    const bool cannot_switch_to_hd_this_frame = !is_ego_lane_matched_ && use_perception_;
    if (cannot_switch_to_hd_this_frame) {
      env_status_->set_hd_map_suppression_reason(
          EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_UNABLE_TO_MATCH_PERCEPTION_WITH_HD_MAP);
      return;
    }

    env_status_->set_perception_map_suppression_reason(EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_EGO_IN_THE_RAMP);
    use_perception_ = false;
    return;
  }

  const bool keep_hd_map_temporarily = !use_perception_ && (ego_is_changing_lane_ || !is_ego_lane_matched_);
  if (keep_hd_map_temporarily) {
    if (ego_is_changing_lane_) {
      env_status_->set_perception_map_suppression_reason(
          EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_EGO_IS_CHANGING_LANE);
  ...
  use_perception_ = true;
  env_status_->set_hd_map_suppression_reason(EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_EGO_NOT_IN_THE_RAMP);
}
  ...
  if (!use_perception_) {
    ...
    if (is_in_area_should_use_hdmap_) {
      ...
      if (is_ego_in_junction_range_) {
        env_status_->set_perception_map_suppression_reason(EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_EGO_IN_JUNCTION);
      } else {
        // 2. 如果当前帧在路口多岔口场景, 维持用图。
        env_status_->set_perception_map_suppression_reason(
            EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_EGO_IN_MULTIPLE_CONSECUTIVE_ROAD_INTERSECTIONS);
      }
    } else {
      // 3. 如果当前帧不在路口范围内,也不在多分岔口场景下，考虑切图。
      if ((distance_to_next_junction_ - BEV_TO_MAP_DISTANCE_THRESHOLD < NEXT_USE_BEV_AREA_DISTANCE_LIMIT) &&
          (next_low_precision_start_offset_ > distance_to_section_over_first_junction_)) {
        // 3.1 本该切图了,
        // 但是距离下一个该切图的位置不足100米的话就别切感知了，怪麻烦
        // 的。当然，如果下一个路口之结束前有低精区域就不要因为怕小麻烦而引来了大麻烦
        env_status_->set_perception_map_suppression_reason(
            EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_EGO_BETWEEN_TWO_VERY_CLOSED_JUNCTIONS);
      } else {
        // 3.2
        ...
        if (!is_ego_lane_matched_) {
          ...
          use_perception_ = true;
          env_status_->set_hd_map_suppression_reason(EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_EGO_PASSED_JUNCTION);
        }
      }
    }
  } else {
    ...
    if (is_in_area_should_use_hdmap_) {
      ...
      if (is_ego_lane_matched_) {
        ...
        if (is_ego_in_junction_range_) {
          env_status_->set_perception_map_suppression_reason(
              EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_EGO_IN_JUNCTION);
        } else {
          env_status_->set_perception_map_suppression_reason(
              EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_EGO_IN_MULTIPLE_CONSECUTIVE_ROAD_INTERSECTIONS);
        }
      } else {
        if (distance_to_next_junction_ < LD_MAP_TO_PERCEPTION_FAILURE_WARNING_LIMIT && is_junction_from_hd_map_) {
          sensor_status_info_.warning_type = SensorWarningType::SWITCH_TO_HD_MAP_FAILURE;
        }
        env_status_->set_hd_map_suppression_reason(
            EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_UNABLE_TO_MATCH_PERCEPTION_WITH_HD_MAP);
      }
      ...
      env_status_->set_hd_map_suppression_reason(
          EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_EGO_NOT_CLOSE_ENOUGH_TO_JUNCTION);
    }
  }
}
...
bool MapSourceSwitch::IsMapDataValid() {
  ...
  if (routing_map == nullptr) {
    env_status_->set_hd_map_suppression_reason(EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_NO_HD_MAP_FRAME);
    return false;
  }
  ...
  if (fusion_mgr_->GetLdSection(front_section_id) == nullptr) {
    // 检查地图route起始点是否正常。
    env_status_->set_hd_map_suppression_reason(EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_NO_NAVIGATION_START_SECTION);
    return false;
  } else {
    // 检查地图剩余长度够不够，是否道路偏航
    ...
    double distance_to_back = front_section->length - routing_map->route.navi_start.s_offset;
    auto   cur_section      = front_section;
    while (distance_to_back <= kMapDataMinAvailableLength) {
      const auto *next_section = GetFirstValidLinkedSection(cur_section->successor_section_id_list, fusion_mgr_);
    ...
    */
      ...
      env_status_->set_hd_map_suppression_reason(EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_APPROACH_MAP_END);
      return false;
    }
  }
...
bool MapSourceSwitch::IsDowngradeCausedByLowPrecisionZone() {
  if (is_on_freeway_) {
    ...
    if (use_perception_ && (env_status_->hd_map_suppression_reason() !=
                            EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_APPROACH_LOW_PRECISION_AREA_IN_HD_MAP)) {
      is_ego_in_junction_range_ = (distance_to_next_junction_ < BEV_TO_MAP_DISTANCE_THRESHOLD);
    } else {
      is_ego_in_junction_range_ = ((distance_to_previous_junction_ < MAP_TO_BEV_DISTANCE_THRESHOLD) ||
                                   (distance_to_next_junction_ < BEV_TO_MAP_DISTANCE_THRESHOLD));
    }
    is_in_area_should_use_hdmap_ = (is_ego_in_junction_range_ || is_ego_in_multi_consecutive_road_intersections_);
  }
  ...
  for (const auto &odd : map_event_ptr_->odd_info) {
    if (cem::message::env_model::ValueType::LOW_PRECISION == odd.value) {
      ...
      next_low_precision_start_offset_ =
          next_low_precision_start_offset_ < odd.start_offset ? next_low_precision_start_offset_ : odd.start_offset;
      if (is_on_freeway_) {
        if (is_in_area_should_use_hdmap_) {
        ...
        if (((distance_to_next_junction_ - BEV_TO_MAP_DISTANCE_THRESHOLD < odd.end_offset) &&
             (distance_to_section_over_first_junction_ + MAP_TO_BEV_DISTANCE_THRESHOLD > odd.start_offset)) ||
        ...
        if (distance_to_previous_junction_ < MAP_TO_BEV_DISTANCE_THRESHOLD) {
          if (odd.start_offset < MAP_TO_BEV_DISTANCE_THRESHOLD - distance_to_previous_junction_) {
            if (min_start_offset > odd.start_offset) {
              min_start_offset = odd.start_offset;
              min_end_offset   = odd.end_offset;
            }
  ...
  if ((min_start_offset < LD_MAP_LOW_PRECISION_DISTANCE_WARNING_LIMIT &&
    ...
    sensor_status_info_.warning_type = cem::message::env_model::SensorWarningType::CLOSE_TO_UNRELIABLE_ROAD;
    return false;
  }
  ...
  if (min_start_offset < LD_MAP_LOW_PRECISION_DISTANCE_DOWNGRADE_LIMIT) {
    ...
    sensor_status_info_.special_type |= static_cast<uint64_t>(SpecialType::ST_LD_MAP_LOW_PRECISION);
    env_status_->set_hd_map_suppression_reason(
        EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_APPROACH_LOW_PRECISION_AREA_IN_HD_MAP);
    return true;
  } else {
...
bool MapSourceSwitch::IsDowngradeCausedByPerceptionMapStillInUseNearJunction() {
  ...
  if (distance_to_next_junction_ < BEV_TO_MAP_DOWNGRADE_DISTANCE_THRESHOLD) {
    ...
    env_status_->set_hd_map_suppression_reason(
        EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_UNABLE_TO_USE_HD_MAP_NEAR_JUNCTION);
    return true;
  }
  return false;
}
...
bool MapSourceSwitch::IsDowngradeCausedByEgoLaneNotMatched() {
  if (!is_on_freeway_ || (use_perception_ && localization_not_reliable_frame_cnt_ <= kLocalizationNotReliableFrameThreshold) ||
      !map_match_status_.valid) {
    localization_not_reliable_frame_cnt_ = 0;
    return false;
  }

  if (localization_not_reliable_frame_cnt_ <= kLocalizationNotReliableFrameThreshold) {
    if ((map_match_status_.location_level != cem::message::sensor::LocationLevel::Roadlevel) || is_ego_lane_matched_) {
      localization_not_reliable_frame_cnt_ = 0;
      return false;
    }
  } else {
  ...
  if (localization_not_reliable_frame_cnt_ > kLocalizationNotReliableFrameThreshold) {
    ...
    env_status_->set_hd_map_suppression_reason(
        EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_LOCALIZATION_IS_NOT_RELIABLE);
    return true;
  } else {
    return false;
  }
...
bool MapSourceSwitch::IsNoaActivated() {
  ...
  if (canoutptr != nullptr) {
    if (canoutptr->DNP_Stats_S != 0x6) {
      env_status_->set_hd_map_suppression_reason(EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_NOT_NOA_MODE);
      return false;
    }
  }
  return true;
...
bool MapSourceSwitch::IsEgoCloseToUnreliableRoad() {
  ...
  if (routing_map == nullptr || is_on_freeway_) {
    ...
    env_status_->set_distance_to_unreliable_road(fusion_mgr_->GetDistanceToUnreliableRoad());
  }
  return false;
}

void MapSourceSwitch::CopySensorStatusInfoTo(cem::fusion::RoutingMapPtr &routing_map) {
  if (routing_map == nullptr) {
    return;
  }
  routing_map->sensor_status_info.sensor_status = sensor_status_info_.sensor_status;
  routing_map->sensor_status_info.special_type  = sensor_status_info_.special_type;
  routing_map->sensor_status_info.warning_type  = sensor_status_info_.warning_type;
}
...
void MapSourceSwitch::ProcessSpecialJunctionInfo() {
  ...
  const SectionInfo *front_section    = fusion_mgr_->GetLdSection(front_section_id);
  ...
  while (true) {
    ...
    distance_to_back += cur_section->length;
    const auto *successor_section = GetFirstValidLinkedSection(cur_section->successor_section_id_list, fusion_mgr_);
    if (successor_section == nullptr) {
      break;
    }
    cur_section = successor_section;
  ...
  distance_to_next_junction_ = distance_to_next_junction_ < 0.0 ? 0.0 : distance_to_next_junction_;
  ...
  while (true) {
    if (distance_to_front >= distance_to_previous_junction_ || distance_to_front > kPreviousJunctionSearchDistance) {
      break;
    }
    const auto *predecessor_section = GetFirstValidLinkedSection(cur_section->predecessor_section_id_list, fusion_mgr_);
...
void MapSourceSwitch::ForceChangeSourceToPerceptionMap() {
  auto force_use_perception = [this](const EnvStatus::MapSuppressionReason reason) {
    ForceUsePerceptionMap(route_snapshot_for_perception_map_switch_failure_, use_perception_, env_status_, reason);
  };
  ...
  auto routing_map = fusion_mgr_->GetLdMapInfo();
  if (routing_map == nullptr) {
    force_use_perception(EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_NO_HD_MAP_FRAME);
    return;
  }

  const auto suppression_reason = env_status_->perception_map_suppression_reason();
  if (suppression_reason == EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_UNABLE_TO_MATCH_PERCEPTION_WITH_HD_MAP) {
    if (next_low_precision_start_offset_ < LD_MAP_LOW_PRECISION_DISTANCE_DOWNGRADE_LIMIT) {
      force_use_perception(
          EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_FORCE_USE_PERCEPTION_MAP_SINCE_LOW_PRECISION_ZONE);
      return;
    }
    ...
    if (front_section == nullptr) {
      force_use_perception(EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_NO_NAVIGATION_START_SECTION);
      return;
    }

    const double front_section_remaining_length = front_section->length - routing_map->route.navi_start.s_offset;

    if (route_snapshot_for_perception_map_switch_failure_.empty()) {
      BuildSwitchFailureRouteSnapshot(fusion_mgr_, front_section, front_section_remaining_length,
                                      route_snapshot_for_perception_map_switch_failure_);
      return;
    }

    double     switch_failure_distance = 0.0;
    const bool is_snapshot_valid       = ComputeSwitchFailureDistance(route_snapshot_for_perception_map_switch_failure_, front_section->id,
                                                                      front_section_remaining_length, &switch_failure_distance);
    if (!is_snapshot_valid) {
      force_use_perception(
          EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_FORCE_USE_PERCEPTION_MAP_SINCE_HISTORY_ROUTE_INVALID);
      return;
    }
    if (switch_failure_distance > LD_MAP_TO_PERCEPTION_FORCE_SWITCH_THRESHOLD) {
      force_use_perception(
          EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_FORCE_USE_PERCEPTION_MAP_SINCE_NOT_MATCHED_WITH_LONG_DISTANCE);
      return;
    }
    return;
  }

  if (suppression_reason == EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_EGO_IS_CHANGING_LANE) {
    if (next_low_precision_start_offset_ < LD_MAP_LOW_PRECISION_DISTANCE_DOWNGRADE_LIMIT) {
      force_use_perception(
          EnvStatus::MapSuppressionReason::EnvStatus_MapSuppressionReason_FORCE_USE_PERCEPTION_MAP_SINCE_LOW_PRECISION_ZONE);
    }
